...
export const WorkflowTrackingModal = ({
  ...
  // completed step detection: prefer to find the last completed step
  const completedStep = steps.slice().reverse().find((s: any) => s.status === 'completed') ?? null;

  // try to read transaction hash and event id from the result if available
  const txHash = completedStep?.result && typeof completedStep.result === 'object' && ('transactionHash' in completedStep.result) ? (completedStep.result as any).transactionHash : null;
  const eventId = completedStep?.result && typeof completedStep.result === 'object' && ('eventId' in completedStep.result) ? (completedStep.result as any).eventId : null;
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader className="text-center">
          <DialogTitle className="flex items-center justify-center gap-2">
            ...
            Create Event {completedStep ? 'Status' : 'in Progress'}
          </DialogTitle>
          <DialogDescription>
            Please wait while we process your event creation request — this may take a few moments.
          </DialogDescription>
        </DialogHeader>
